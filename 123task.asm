...
; Tasks    : Task 1 - Display original matrix  (non-digits in RED)
;            Task 2 - Clean matrix             (replaced zeros in RED)
;            Task 3 - Normalize matrix         (digit<5->0 , digit>=5->1)
; Tool     : GUI Turbo Assembler x64  (TASM 4.1 / TLINK)
; Fix log  : - IDEAL + explicit SEGMENT/ENDS fixes "@DATA undefined" error
;            - TOTAL_CELLS EQU 28 fixes "expression in MOV" TASM error
;            - PrintTwoDigitsSafe saves units digit in CL before INT 10h
;            - dt_* memory vars used in PrintDateTime (no stack confusion)
;            - ES initialised in MAIN (required for STOSB)
;===========================================================================

IDEAL                        ; TASM Ideal mode  (resolves @DATA issues)
MODEL  SMALL
STACK  200h
...
PROC MAIN
    ...
    MOV  DS, AX
    MOV  ES, AX              ; ES = DS  (needed for STOSB in Tasks 2 & 3)
