...
PROC Timer1Ch_Handler
    ...
    INC  WORD [tick_count]    ; count this tick

    CMP  WORD [tick_count], TICKS_30SEC
    JB   @@ISR_exit           ; not 30 seconds yet

    MOV  WORD [tick_count], 0 ; reset counter
...
PROC MAIN
    ...
    ;--- Main wait loop ---------------------------------------------------
@@MainLoop:
    CMP  BYTE [task_flag], 0
    JE   @@MainLoop            ; idle until ISR sets the flag

    MOV  BYTE [task_flag], 0   ; clear flag before dispatching
...
@@AfterTask:
    INC  BYTE [task_index]
    CMP  BYTE [task_index], TOTAL_TASKS
...
;===========================================================================
; Additional data variables needed by tasks
; (appended to _DATA using a second SEGMENT block - valid in IDEAL mode)
;===========================================================================
SEGMENT _DATA
    row_sum  DB 0             ; scratch byte: current row sum for Tasks 4 & 5
ENDS _DATA
